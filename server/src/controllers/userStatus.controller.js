...
const {
  certificationSendMailTemplate,
} = require("../utils/CertificationSendMailTemplate.utils");

let userStatusPost = async (req, res) => {
  console.log("Status Post Function");
  console.log(
    "Request body from cruddddd Controller jjjjjjjjjjjjjjjjjjj",
    req.body
  );

  const { skills, tips, collegeName, studentName, displayPicture, date } =
    req.body;
...
let updateCertification = async (req, res) => {
  console.log("Update Status Function");

  const { approval } = req.body;
  const approve = approval === "Not Approved" ? "false" : "true";
...
let updateWorkingQuestion = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      tips,
      approval: data.approval,
      message: "Updated Working Questions Successfully",
    });
  } catch (err) {
    console.error(err);
    return res.status(500).send("Internal Server Error");
...
const ratings = async (req, res) => {
  ...
  try {
    ...
    const existingRatingIndex = userStatus.rating.findIndex(
...
module.exports = {
  updateCertification,
  userStatusPost,
  getUserWorkingQuestions,
  updateApproval,
  updateWorkingQuestion,
  getTips,
  tipViews,
  ratings,
  getUserWorkingQuestionsAdmin,
};
